	...
	return &schema.Provider{
		Schema: map[string]*schema.Schema{
			...
			"token": &schema.Schema{
				...
				Optional:     true,
				Sensitive:    true,
				Description:  "Zabbix API token. Used instead of username and password: the provider sends it directly and skips the login call. Falls back to $ZABBIX_TOKEN",
				ValidateFunc: validation.StringIsNotWhiteSpace,
		...
		DataSourcesMap: map[string]*schema.Resource{
			"zabbix_action_trigger": dataActionTrigger(),
		...
		ResourcesMap: map[string]*schema.Resource{
			"zabbix_action_trigger": resourceActionTrigger(),
